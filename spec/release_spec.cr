...
describe Release do
  it "#add_actor" do
    r = Release.new
    r.add_actor("John Doe", "discogs", "12345")
    r.actors.size.should eq 1
    r.add_actor("John Doe", "discogs", "12345")
    r.actors.size.should eq 1
    r.add_actor("Cpt. Nemo", "musicbrainz", "guid")
  ...
  it "#compare" do
    ...
    t2 = Track.new("", index = 0)
    t2.title = "Some Prince will come"
    r2.tracks << t2
    r.tracks.compare(r2.tracks).should eq 1.0
    r.pub.add_label_catno("test", "test_catno")
    r.pub.labels.has_label("test").should be_true
    r2.pub.add_label_catno("test", "test_catno")
  ...
  it "#moods" do
    r = Release.new
    t = Track.new("01")
    t.record.moods << Mood::CALM
    r.tracks << t
    t2 = Track.new("02")
    t2.record.moods << Mood::ENERGETIC
    r.tracks << t2
  ...
  it "#no_mandatory_fields" do
    ...
    r.title = "test title"
    r.no_mandatory_fields.should be_true
    r.roles.add_role("Nemo", "performer")
    r.no_mandatory_fields.should be_true
    r.tracks << Track.new("01")
    r.no_mandatory_fields.should be_true
    r.pub.labels << Label.new("test")
  ...
  it "#pub_ancestor" do
    r = Release.new
    r.pub_ancestor.empty?.should be_true
    r.pub.add_label(Label.new("test"))
    r.pub_ancestor.empty?.should be_true
  end
  ...
  it "#pub_compare" do
    ...
    r2 = Release.new
    r2.pub.year = 2000
    r2.pub.add_label(Label.new("test"))
